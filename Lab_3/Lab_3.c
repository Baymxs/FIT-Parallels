...
/* DIMS_NUMS - размер декартовой топологии. P0xP1*/
#define DIMS_NUM 2
...
int main(int argc, char *argv[]) {
    ...
    double p[2]; // размер двумерной решетки: p[0] = dims[0] x p[1] = dims[1]
    ...
    nn[0] = n[0] / p[0]; // находим размер подматрицы AA для p[0] процессов
    nn[1] = n[2] / p[1]; // находим размер подматрицы BB для p[1] процессов
    ...
    for(int i = 0; i < nn[0]; i++){
        for(int j = 0; j < nn[1]; j++){
            CC[nn[1]*i + j] = 0.0;
            for(int k = 0; k < n[1]; k++){
                CC[nn[1]*i+j] = CC[nn[1]*i+k] + AA[n[1]*i+k] * BB[nn[1]*j+k];
            }
        }
    }
    ...
    if (current_process!=0) {
         ...
         */
        //Передаем подматрицы СС из каждого ненулевого процесса нулевому процессу
        MPI_Send(СС, nn[0] * nn[1], MPI_DOUBLE, 0, 0, grid_comm);
    }
    ...
    if (current_process == 0) {
        printf("%Processes: %d\n Time: %lf\n", number_of_processes, finish_time - start_time);
